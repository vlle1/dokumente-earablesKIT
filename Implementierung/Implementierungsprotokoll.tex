...
\paragraph{Hinzugefügt in der Klasse EarablesConnection}
\begin{itemize}
	\item[-] CheckConnection():void Anfang aller  Methode, die eine Verbindung voraussetzen, aufgerufen und prüft ob eine Verbindung existiert. Falls keine existiert schmeißt sie eine die Exception NoConnectionException.
\end{itemize} 
Die Methode CheckConnection wird am Anfang aller Methoden, die eine Verbindung voraussetzen, aufgerufen und prüft, ob eine Verbindung existiert. Falls keine existiert schmeißt sie die Exception NoConnectionException.
...
\paragraph{Hinzugefügt in der Klasse ConfigContainer}
\begin{itemize}
	\item[?] AccScaleFactor: int
	\item[?] GyroScaleFactor: int
	\item[?] private setSamplingRate(int rate): void
\end{itemize}
...
\paragraph{Beschreibung}
Der StepMode muss gleichzeitig von zwei Aktivitätsanalysen Events bekommen, von der RunningActivity und von der StepActivity. Wenn die OnActivityDone Methode bei beiden Event Handlern der Aktivitäten registriert wird, müssten mehrere Fallunterscheidungen und eine Auswertung der nicht implementierten ActivityArgs erfolgen. Stattdessen wird eine neue Methode hinzugefügt, sodass bei jedem Event Handler nur eine Methode registiert ist.
\paragraph{Änderungen in StepModeViewModel}
\begin{itemize}

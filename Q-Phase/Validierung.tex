...
\newcommand{\testcorrected}[0]{
	\cellcolor{blue!25} CORRECTED
}
...
Wir benutzen dabei die Testszenarien, die im Pflichtenheft spezifiziert wurden, um die korrekte Funktionsweise unserer App zu überprüfen.
Zusätzlich wurden xUnit-Tests geschrieben, die zur Überprüfung einzelner Module dienen.
Die Testüberdeckung wurde mit ReSharper ermittelt und im Dokument durch aussagekräftige Diagramme visualisiert.
Für unsere Bibliothek wurde zusätzlich eine Test App entwickelt, die uns mit neu angefertigten Szenarien dabei hilft, die Korrektheit sicher zu stellen.\\
Außerdem haben wir fünf Probanden gefunden, die sich bereiterklärt haben, die Benutzerfreundlichkeit und Funktionalität unserer App zu überprüfen (siehe Nutzertests).
...
\paragraph{Methodik}
Die Testszenarien wurden von drei verschiedenen an der Entwicklung beteiligten Personen auf drei verschiedenen Android-Geräten durchgeführt. Wir haben die Tests mehrmals im Verlauf der Qualitätssicherungsphase durchgeführt. Tests, die nicht von Anfang an erfolgreich verliefen, sind markiert.
\paragraph{Legende} 
\begin{itemize}
	\item OK: In allen Testläufen erfolgreich
	\item CORRECTED: In mindestens einem Testlauf nicht erfolgreich
\end{itemize}
\clearpage
...
Bei T200 wurde wurde ein Tolernazbereich hinzugefügt, der ursprünglich nicht berücksichtigt wurde.
...
\begin{tabular}{ |p{1.5cm} | p{12cm} | c| }
	...
	\hline
	/T200/ & Die App zeigt 15 an (mit einer Toleranz von 10$\%$). & \testcorrected  \\
	\hline
\end{tabular}
\paragraph{Anmerkungen}
Die App zeigte beim ersten Fall von 3 Fällen am Ende nicht exakt 15 an.\\
Bei SitUps wurden im ersten Fall nur 13 erkannt. Bei den folgenden Tests mit korrekterer Ausführung der Übung war das Ergebnis im erlaubten Toleranzbereich.
\clearpage
\subsection{Start/Stop Musikmodus}
\paragraph{Änderungen}
Die Funktion wurde aus dem Laufmodus in den Modus \glqq Musik Modus\grqq{} umbenannt.\\
Der Musikmodus interagiert nicht mit einem externen Musik-Player, sondern spielt selbst Musik ab (siehe Implementierungsprotokoll). Dementsprechend wurde die Reihenfolge der Testfälle angepasst.
\\
\\
\begin{tabular}{ |p{1.5cm} | p{12cm} | c| }
	\hline
	...
	\hline
	& Die Musik startet automatisch. & \testok  \\
...
\begin{tabular}{ |p{1.5cm} | p{12cm} | c| }
	...
	\hline
	& Der Vorgang wird durch den Nutzer beendet. & \testok \\
	\hline
\end{tabular}
...
\begin{tabular}{ |p{1.5cm} | p{12cm} | c| }
	...
	\hline
	& Die Einstellungen sind exakt so, wie sie vorher eingestellt wurden. & \testcorrected  \\
	\hline
\end{tabular}
\paragraph{Anmerkungen}
...
\paragraph{Änderungen}
\glqq Import/Export\grqq{} wurde in \glqq Dateimanagement\grqq umbenannt.
\\
\\
\begin{tabular}{ |p{1.5cm} | p{12cm} | c| }
	\hline
...
\end{tabular}
\paragraph{Anmerkungen}
Zu T270: Bei einem Testdurchlauf kam es zu einem nicht reproduzierbaren Absturz der App (beim Klicken auf Löschen). Die Daten wurden allerdings korrekt gelöscht.
...
Bei unserer Bibliothek haben wir mit xUnit-Tests alles getestet, was ohne eine Verbindung mit den Earables möglich war. Dies beinhaltet die Klassen IMUDataEntry, Gyroskop, Accelerometer und IMUDataExtractor.
\\ \\ 
\includegraphics[width=1\textwidth]{./bilder/BibTestCoverage/Testcoverage.PNG}
\subsection{Model}
Bei unserem Model haben wir nahezu alles getestet. Die Algorithmen haben wir mit voraufgezeichneten Daten getestet indem die Anzahl der erkannten Ausführungen mit der Zahl der Ausführungen bei der Aufzeichnung verglichen wurden. So konnten wir die Algorithmen mit gemockten Daten auf Korrektheit testen.\\
Zudem haben wir bei allen Tests mit der internen Reflection gearbeitet. Somit konnten wir den Serviceprovider im Servicemanager mocken. Über diesen können dann die gemockten Services zurückgegeben werden, damit die zu testende Klasse ohne Abhängigkeiten getestet werden kann. Dieses Vorgehen wurde beispielsweise beim SettingsService angewandt.\\

\includegraphics[width=0.85\textwidth]{./bilder/Coverage/CoverageModel.PNG}

\subsection{ViewModel}
Wir haben jedes ViewModel getestet. Dabei haben wir eine Testüberdeckung von 100\% angepeilt. Diese konnten wir nicht in jedem ViewModel erreichen, da es statische Methoden und Properties gibt, die nicht gemockt werden können. Bei diesen handelt es sich oft um spezifische Device Methoden.
\\Auch im Viewmodel haben wir mit der internen Reflection gearbeitet, da jedes Viewmodel eine Relation zu mindestens einem anderen Service besitzt.\\
\newline
...
Da wir für die Bluetoothverbindung in unserer Bibliothek das NuGet \glqq{}lBluetooth LE plugin for Xamarin\grqq{} benutzen, erschwert uns dies das Testen mit xUnit tests. Wir können hier nicht, wie beim Rest des Projektes, mit Reflection arbeiten, weil es sich bei der \glqq{}obersten Klasse\grqq{} um eine statische Klasse handelt. Diese besitzt nur eine statische Property, welche man nicht mocken kann, da kein Setter bereitgestellt wird.\\
 Um unsere Bibliothek trotzdem zu testen, haben wir uns dafür entschieden eine eigene Test App für die Bibliothek zu bauen. So sind wir in der Lage, mit kleinen Szenarien, die nur unsere Bibliothek betreffen, zielgerichtete Tests durchzuführen.
...
\begin{itemize}
	...
	\item das Setzen des Low Pass Filter (LPF) für das Gyroskop
...
\paragraph{Hinweis}Für die nachfolgenden Szenarien wird angenommen, dass die App bereits gestartet ist und eine Verbindung zu den Earables besteht.
...
\subsubsection{Setzen des LPF für das Gyroskop}
\begin{tabular}{ | p{12cm} | c| }
	\hline
	\textbf{Beschreibung} & \textbf{Bestanden}\\
	\hline
	...
	\hline
	Es wird angezeigt dass der LPF für das Gyroskop 5Hz beträgt . & \testok \\
	\hline
	...
	Der Nutzer drückt den Button \glqq Get Gyro LPF\grqq{}. & \testok \\
	\hline
	Es wird angezeigt, dass der LPF für das Gyroskop 41Hz beträgt . & \testok \\
	\hline
\end{tabular}
\\ \\ \\ \\
\begin{figure}[h]
	\centering
    \subfigure[App fordert den Nutzer nach Drücken des Buttons \glqq Set Gyro LPF\grqq{} auf, einen LPF für das Gyroskop  auszuwählen]{\includegraphics[width=0.4\textwidth]{./bilder/BibTestApp/GyroLPF.jpeg}}
\end{figure}
\FloatBarrier
...
\begin{figure}[h]
	\centering
    \subfigure[App fordert den Nutzer nach Drücken des Buttons \glqq{Set Acc LPF}\grqq{} auf, einen LPF für das Accelerometer auszuwählen]{\includegraphics[width=0.4\textwidth]{./bilder/BibTestApp/AccLPF.jpeg}}
\end{figure}
\FloatBarrier
...
\begin{figure}[h]
	\centering
    \subfigure[App fordert den Nutzer nach Drücken des Buttons \glqq{Set Samplingrate}\grqq{} auf, eine gültige Samplingrate einzugeben]{\includegraphics[width=0.4\textwidth]{./bilder/BibTestApp/SetSamplerate.jpeg}}
\end{figure}
\FloatBarrier

\subsubsection{Wertebereich des Gyroskops verändern}
\begin{tabular}{ | p{12cm} | c| }
	\hline
	\textbf{Beschreibung} & \textbf{Bestanden}\\
	\hline
...
\paragraph{Hinweis}
Der Scale Factor hängt von der gewählten Range ab (die Skalierung der Daten, die die Earables senden, ändert sich je nach eingestellter Range).
\\ \\ \\ \\
\begin{figure}[h]
	\centering
    \subfigure[App fordert den Nutzer nach Drücken des Buttons \glqq{Set Gyro Range}\grqq{} auf, einen Range für das Gyroskop auszuwählen]{\includegraphics[width=0.4\textwidth]{./bilder/BibTestApp/GyroRange.jpeg}}
\end{figure}
\FloatBarrier

\subsubsection{Wertebereich des Beschleunigungssensors verändern}
\begin{tabular}{ | p{12cm} | c| }
	\hline
	\textbf{Beschreibung} & \textbf{Bestanden}\\
	\hline
	...
	Der Nutzer drückt den Button \glqq{}Start Sampling\grqq{}. & \testok \\
	\hline
	Es werden die Rohdaten auf dem Bildschirm angezeigt und der Nutzer erkennt, dass die Range der Beschleunigungen auf den Achsen  auf $\pm 4$ g beschränkt ist, wenn er sich bewegt. & \testok \\
	\hline
	Der Nutzer drückt den Button \glqq{}Stop Sampling\grqq{}. & \testok \\
	\hline
	Das Sampling wird gestoppt und die Rohdaten hören auf sich zu aktualisieren. & \testok \\
	...
	Der Nutzer drückt den Button \glqq{}Start Sampling\grqq{}. & \testok \\
	\hline
	Es werden die Rohdaten auf dem Bildschirm angezeigt und der Nutzer erkennt, dass die Range der Beschleunigungen auf den Achsen  auf $\pm 2$ g beschränkt ist, wenn er sich bewegt. & \testok \\
	\hline
	Der Nutzer drückt den Button \glqq{}Stop Sampling\grqq{}. & \testok \\
	\hline
\end{tabular}
...
\begin{figure}[h]
	\centering
    \subfigure[App fordert den Nutzer nach dem Drücken des Buttons \glqq{Set Acc Range}\grqq{} auf, eine Range für das Accelerometer auszuwählen]{\includegraphics[width=0.4\textwidth]{./bilder/BibTestApp/AccRange.jpeg}}
\end{figure}
\FloatBarrier
\clearpage
\subsubsection{Akkuspannung auslesen}
\begin{tabular}{ | p{12cm} | c| }
	\hline
	\textbf{Beschreibung} & \textbf{Bestanden}\\
	\hline
...
\begin{figure}[h]
	\centering
    \subfigure[App zeigt dem Nutzer, nach Drücken des Buttons \glqq{Set Battervoltage}\grqq{}, den aktuellen Wert der Batterievotage an]{\includegraphics[width=0.4\textwidth]{./bilder/BibTestApp/BatteryVoltage.jpeg}}
\end{figure}
\FloatBarrier
\clearpage
\subsubsection{Pushbutton drücken}
\begin{tabular}{ | p{12cm} | c| }
	\hline
	\textbf{Beschreibung} & \textbf{Bestanden}\\
	\hline
...
\subsubsection{Bluetoothstatus auslesen}
\begin{tabular}{ | p{12cm} | c| }
	\hline
	\textbf{Beschreibung} & \textbf{Bestanden}\\
	\hline
...
\section{Nutzertests}
\paragraph{Einleitung}
Diese Tests wurden mit Testpersonen ausgeführt, die nicht an der Entwicklung der App beteiligt waren und somit nicht mit der App vertraut waren. Wir haben dabei fünf Testpersonen ($P_1 \text{ bis } P_5$) die gleichen Abläufe in der App durchlaufen lassen. Bei den Testpersonen handelt es sich um durchschnittlich sportliche Personen zwischen 16 und 27 Jahren. Es wurde bei den Tests darauf geachtet, dass die Sportlichkeit der Personen eine möglichst geringe Rolle spielt, daher wurden diese Informationen nicht zusätzlich festgehalten.
...
Bei $P_2$ wurden die ersten drei Liegestütze sehr schnell durchgeführt, weswegen sie nur als eine gezählt wurden.\\
Bei $P_3$ wurden die Liegestütze sehr langsam ausgeführt, weshalb nicht alle erkannt wurden.\\
...
Insgesamt hat sich herausgestellt, dass die Geschwindigkeit der Ausführung einen Einfluss auf die korrekte Erkennung hat.
...
Bei $P_3$ wurden die SitUps sehr langsam ausgeführt, weshalb nicht alle erkannt wurden.\\
...
Insgesamt kann gesagt werden, dass die Earables beim Starten des Samplings eine gewisse Verzögerung (ca. eine Sekunde) hatten, bis sie tatsächlich Daten senden. Daher werden oft die ersten Schrite nicht erkannt. 
...
Die Erstellung des Trainingsplans hat bei $P_2$ ohne Probleme funktioniert. Es wurden von 18 Sit-Ups zwei erkannt. Bei den Liegestützen hat $P_2$ die ersten drei Liegestützen sehr schnell durchgeführt, weswegen sie nur als eine gezählt wurden.\\
$P_4$ hat seinen Trainingsplan selbstständig eingestellt und hat die Übungen durchgeführt. Die SitUps wurden nicht richtig erkannt; es waren vier zu wenig. Diese wurden dann simuliert, um die weiteren Funktionen zu testen.\\
...
\section{In der Qualitätssicherungsphase gefixte Bugs}
\paragraph{Anmerkung}
Im Folgenden werden nicht alle gefixten Bugs aufgelistet, sondern nur eine Auswahl an interessanten oder relevanten Bugs.


\subsection{Dialog \glqq{}Edit\grqq{} in Listen\&Perform}
\begin{itemize}
	\item Dialog wegklicken führte zu Löschung, sollte aber da bleiben (Grund: als Edit-Dialog wurde der Add-Dialog verwendet).
	\item Dialog zeigte \glqq{}add activity\grqq{} statt \glqq{}edit activity\grqq{}. Jetzt wird stattdessen einfach der Name der Aktivität angezeigt, was deutlich intuitiver ist.
\end{itemize}

\subsection{Resource Stings: sprachliche Eigenheiten}
Bestimmte Sätze haben im Englischen/Deutschen eine andere grammatikalische Struktur, daher haben wir das Problem mit aufgeteilten Strings behoben. Beispiel: \glqq{}Du hast 12 Schritte gemacht\grqq{} bzw. \glqq{}You have taken 12 Steps\grqq{}.

\subsection{Speicherung der Daten in Listen\&Perform}
Ausführungen von Liegestütze wurden nicht korrekt gespeichert (aber korrekt runtergezählt während Ausführung), aber nur wenn die App auf Englisch eingestellt war. SitUps werden korrekt gespeichert, der Bug tritt im Countmode nicht auf.

\subsection{Anzeige der Schrittfrequenz}
Die Schrittfrequenz wurde sehr ungenau berechnet, da nur alle drei Sekunden die Schritte der letzten drei Sekunden betrachtet wurden.\\
Stattdessen wird jetzt lineare Regression verwendet.

\subsection{Speicherung der SamplingRate}
Einstellen der SamplingRate funktionierte nicht zuverlässig, da die Kommunikation in der App an die Earables nicht richtig implementiert war.

\subsection{Label des Timers im Zählmodus springt hin und her}
Je nach Anzahl der Stellen der Millisekunden hat sich das Label verschoben und war so schwer leserlich.\\
Jetzt werden die Stellen mit führenden Nullen aufgefüllt.

\subsection{Verbesserung: Nutzerfeedback bei Verbindung}
Während dem Herstellen der Verbindung mit den Earables wurde keinerlei Hinweis angezeigt, dass dies manchmal ein bisschen länger dauern kann (wenige Sekunden). Nutzer sind das nicht gewohnt, daher wurde dies ergänzt.
